...
## 🔍 CrewAI - 2 chế độ hoạt động

### **Chế độ 1: Direct Serper Search (Độc lập - Khuyến nghị)**
- **Chỉ cần**: Serper API key
- **Không cần**: Gemini/OpenAI/Llama API
- **Tính năng**: Tìm kiếm tin tức trực tiếp từ Serper
- **Ưu điểm**: Nhanh, đơn giản, không phụ thuộc LLM
- **Phù hợp**: Người dùng chỉ cần tin tức thật

### **Chế độ 2: LLM-Enhanced Search (Nâng cao)**
- **Cần**: Serper API + (Gemini hoặc OpenAI hoặc Llama)
- **Tính năng**: AI phân tích và tóm tắt tin tức
- **Ưu điểm**: Phân tích sâu hơn, sentiment analysis
- **Phù hợp**: Người dùng muốn AI giúp phân tích
...
- **Gemini API key** (tùy chọn, miễn phí tại [Google AI Studio](https://aistudio.google.com/apikey))
- **Serper API key** (khuyến nghị cho tin tức thật, tại [Serper.dev](https://serper.dev/api-key))
- Click **"🔧 Cài đặt Gemini"** hoặc **"🚀 Cài đặt CrewAI"**

**Lưu ý**: CrewAI có thể hoạt động độc lập với chỉ Serper API key (không cần Gemini/OpenAI/Llama)
