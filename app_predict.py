...
def display_architecture_prediction_tables(pred, symbol, architecture):
    ...
    with tab1:
        # Chỉ hiển thị ngày cuối cùng (3 ngày)
        final_date = datetime.now() + timedelta(days=3)
        st.subheader("📊 Dự đoán Ngắn hạn (3 ngày)")
        
        # Tạo DataFrame với 1 dòng duy nhất
        import pandas as pd
        df_short = pd.DataFrame([[
            format_prediction_date(final_date),
            f"{short_price:,.2f} VND"
        ]], columns=['Ngày dự đoán', 'Giá dự đoán'])
        
        st.dataframe(df_short, use_container_width=True, hide_index=True)

    with tab2:
        # Chỉ hiển thị ngày cuối cùng (14 ngày)
        final_date = datetime.now() + timedelta(days=14)
        st.subheader("📈 Dự đoán Trung hạn (14 ngày)")
        
        # Tạo DataFrame với 1 dòng duy nhất
        df_medium = pd.DataFrame([[
            format_prediction_date(final_date),
            f"{medium_price:,.2f} VND"
        ]], columns=['Ngày dự đoán', 'Giá dự đoán'])
        
        st.dataframe(df_medium, use_container_width=True, hide_index=True)

    with tab3:
        # Chỉ hiển thị ngày cuối cùng (60 ngày)
